import { UserVibes, VIBE_PACKS, VibePackName } from "@/lib/types/vibes";
...
export function vibesToPrompt(vibes: UserVibes, tripId?: string): string {
  ...
  const paceDesc =
    comfort.pace_score <= 30
      ? "relaxed/slow"
      : comfort.pace_score <= 60
      ? "moderate"
      : "active/intense";

  const daypartDesc =
    comfort.daypart_bias === "early"
      ? "early morning starts"
      : comfort.daypart_bias === "late"
      ? "late starts, evening activities"
      : "flexible timing";

  const foodDesc =
    taste.food_adventurousness <= 2
      ? "familiar, safe options"
      : taste.food_adventurousness <= 3
      ? "regional classics"
      : taste.food_adventurousness <= 4
      ? "adventurous local cuisine"
      : "experimental and wild food experiences";

  const crowdDesc =
    logistics.crowd_tolerance <= 2
      ? "avoid lines and crowds"
      : logistics.crowd_tolerance <= 3
      ? "okay with some crowds"
...
- Surprise level: ${
    logistics.surprise_level === 0
      ? "no surprises"
      : logistics.surprise_level === 1
      ? "few surprises"
      : "surprise me daily"
  }`;
...
- ${
    logistics.crowd_tolerance <= 2
      ? "Prioritize off-peak times and lesser-known alternatives"
      : "Include popular attractions"
  }
- ${
    logistics.surprise_level === 0
      ? "Only suggest well-known, predictable activities"
      : logistics.surprise_level === 1
      ? "Include 1 surprise/hidden gem per day"
      : "Include 2+ surprises per day"
  }`;
 ...
 */
export function applyVibePack(
  vibes: UserVibes,
  packName: VibePackName
): UserVibes {
...
export function hasCompletedVibes(vibes: UserVibes | null): boolean {
  ...
  return (
    vibes.vibe_packs.length > 0 ||
    Object.keys(vibes.taste.theme_weights).length > 0
  );
}
