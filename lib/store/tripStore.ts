...
const getInitialState = () => {
  ...
  if (saved) {
    return {
      ...
      userVibes: saved.userVibes || null,
    }
  }
...
export const useTripStore = create<TripStore>()(
  temporal(
    immer((set, get) => ({
      ...
      getAllTrips: () => {
        const state = get()
        return Object.values(state.trips).sort((a, b) => 
          ...
          if (trip) {
            if (dayId === 'unassigned') {
              if (trip.unassignedCards) {
                trip.unassignedCards = trip.unassignedCards.filter(c => c.id !== cardId)
              }
            } else {
              const day = trip.days.find(d => d.id === dayId)
              if (day) {
      ...
      moveCard: (tripId, fromDayId, toDayId, cardId, newIndex) => {
        set((state) => {
          ...
          if (trip) {
            ...
            const fromDay = fromDayId === 'unassigned' 
              ? null 
              : trip.days.find(d => d.id === fromDayId)
            const toDay = toDayId === 'unassigned' 
              ? null 
            ...
            if (fromDayId === 'unassigned') {
              if (!trip.unassignedCards) trip.unassignedCards = []
              cardIndex = trip.unassignedCards.findIndex(c => c.id === cardId)
              if (cardIndex !== -1) {
