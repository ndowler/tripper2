...
export function CommandPalette({
  ...
  const handleSelectTemplate = useCallback(
    ...
    [addCard, defaultDayId, onClose, tripId]
  );
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      const items =
        view === "templates" ? filteredTemplates : AI_SUGGESTION_CATEGORIES;
  ...
  const handleAddSelectedSuggestions = () => {
    const suggestionsToAdd = aiSuggestions.filter((_, index) =>
    ...
    toast.success(
      `Added ${suggestionsToAdd.length} suggestion${
        suggestionsToAdd.length !== 1 ? "s" : ""
      }`
    );
    onClose();
    setSearch("");
        ...
        <div className="bg-card border rounded-lg shadow-2xl overflow-hidden">
                  ...
                  <>
                    ...
                    <div className="px-4 py-3 border-t bg-muted/30">
                      <Button
                        ...
                        className="w-full"
                      >
                        Add {selectedSuggestions.size} Selected Suggestion
          ...
          <div className="border-t px-4 py-2 bg-muted/30 flex items-center justify-between text-xs text-muted-foreground">
            <div className="flex gap-4">
              ...
              <span>
                <kbd className="px-1.5 py-0.5 bg-background rounded">Tab</kbd>{" "}
                Switch view
              </span>
            </div>
            <span>
